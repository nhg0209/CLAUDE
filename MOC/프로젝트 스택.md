...
| 16 | ★ residual 의 base 를 스택 `Controller.py` 로 할 것인가 | **아니다. 순수 PP.** `Controller.py` 는 **비교군**으로 남긴다 | 코드를 직접 읽고 확인: 내부 상태 4개(`filtered_heading_error` EMA, `heading_error_integral` 무한누적 PID, `prev_heading_error`, `_speed_cmd_prev` slew)와 상태머신 게인 점프(`OVERTAKE` 시 `*=0.65`). **관측에 없는 상태가 base 출력을 바꾸면 residual 입장에서 non-Markovian** — imprecise 가 아니라 unobservable 이라 더 나쁘다. 생성자 인자 40개, 배치 불가(256 env 불가). 순수 PP 는 torch 10줄 |
| 17 | 종방향 base 를 무엇으로 채우나 | $v_{\text{ref}} = \sqrt{a_{\text{lat,max}}/\lvert\kappa\rvert}$, $a_{\text{lat,max}} = 3.5$ 고정 | **PP 는 횡방향 전용**이라 $a_x$ base 가 비어 있다. `vx_planner` 는 ggv 자리표시자라 못 쓴다(#…). 곡률만 보는 교과서 마찰원 = 파라미터 1개. **residual 에서 base 는 상한이 아니라 출발점**이므로 "vx 를 상한으로 쓰지 말라"와 충돌하지 않는다 ($\Delta a_\theta > 0$ 이면 넘어선다). 3.5 는 $\mu{=}0.357$ 에 해당 — 커리큘럼 주 구간에서 실현 가능하고 꼬리에서만 감속을 배우게 된다 |
