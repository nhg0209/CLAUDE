...
#### 8-5-1. ★ 정량 확인 — steady-state 를 직접 풀었다 ✅ (2026-09-14)

`vehicle_dynamics_st` 의 $\ddot\psi=0,\ \dot\beta=0$ 을 풀어 **$\mu$ 별 필요 조향각**을 구했다.
조건: $v=5\,\mathrm{m/s}$, $R=8\,\mathrm{m}$ ($a_y=3.12\,\mathrm{m/s^2}$), $a_x=0$, `CAR/dynamics.yaml` 파라미터.

| $\mu$ | 필요 $\delta$ [deg] | $\beta$ [deg] | `s_max`(24°) 대비 | 실제 $a_{y,\max}=\mu g$ | 물리적 판정 |
|---|---|---|---|---|---|
| 1.049 | 2.86 | −1.96 | 11.9% | 10.3 | 여유 70% |
| 0.800 | 3.02 | −2.95 | 12.6% | 7.8 | 여유 60% |
| 0.600 | 3.24 | −4.35 | 13.5% | 5.9 | 여유 47% |
| 0.400 | 3.67 | −7.13 | 15.3% | 3.9 | ⚠️ 한계의 80% |
| **0.32** | ~3.9 | ~−9 | ~16% | **3.1** | ⚠️ **물리적 한계** |
| 0.250 | 4.46 | −12.15 | 18.6% | 2.5 | ❌ **불가능** |
| 0.150 | 5.85 | −21.07 | 24.4% | 1.5 | ❌ 불가능 |
| 0.080 | **8.91** | **−40.59** | 37.1% | 0.8 | ❌ 불가능 |

**$\mu$ 가 약분되지 않는 이유**도 코드에서 확인했다 — $\dot\beta$ 식의 **kinematic 결합항 $-1$ 하나뿐**이다:
```
β̇ = ( μ/(v²L)·(C_Sr·lr − C_Sf·lf) − 1 )·ψ̇ − ...
                                    ↑ 이 −1 만 μ 에 안 붙는다
```
$\ddot\psi$ 식은 **모든 항에 $\mu$** 가 붙어 $\ddot\psi=0$ 에서 통째로 약분된다.
→ 마찰 효과가 오직 이 한 항으로만 들어와 **깔끔한 단조 understeer** 가 나온다.

> [!danger] ★★ 이 표가 보여주는 진짜 위험
> **오른쪽 두 열이 왼쪽과 모순된다.**
> - 모델: $\mu=0.08$ 에서도 $\delta=8.9°$ 면 그 코너를 돈다고 답한다. $\beta=-40.6°$ 인데도.
> - 물리: $\mu<0.32$ 에서 그 코너는 **어떤 조향각으로도 못 돈다.**
> - 극한: $\mu\to0 \Rightarrow \delta\approx\frac{vL}{\mu}\cdot\frac{\dot\psi}{C_f}\to\infty$ — **포화가 없다.**
>
> **실차에서는 한계를 넘는 순간 부호가 뒤집힌다** — 앞타이어가 먼저 포화되어 **더 꺾을수록 횡력이 줄고 언더스티어가 심해진다.**
>
> ⭐ **그런데 Tier 1 은 그 반전을 절대 보여주지 않는다.**
> → policy 는 *"미끄러우면 더 꺾어라"* 를 **무한히 참인 법칙**으로 배운다.
> → 실차에서 **처음 한계를 넘는 순간 정확히 틀린 방향으로 행동한다.**
> → §12-4 리스크 #2 의 정량적 근거. **Tier 2 필수화 또는 포화 타이어 모델 교체.**

#### 8-5-2. ★★ Tier 2 에서 포화를 실증했다 (2026-09-15, Isaac Sim 5.1 / PhysX)

`tools/test_racecar_drive.py` 로 같은 토크를 주고 $\mu$ 만 바꿔 직진 최대 가속을 측정했다.

| $\mu$ | $a$ [m/s²] | $\mu g$ | $a/\mu g$ | 종방향 슬립비 | 판정 |
|---|---|---|---|---|---|
| 1.00 | 4.882 | 9.810 | 0.50 | 0.000 | 토크 제한 |
| 0.50 | 4.415 | 4.905 | 0.90 | 0.026 | 중간 |
| **0.25** | **2.441** | **2.453** | **1.00** | **0.481** | **마찰 제한** ✅ |
| **0.12** | **1.173** | **1.177** | **1.00** | **0.766** | **마찰 제한** ✅ |

**$\mu$ 비 2.08 : 가속도 비 2.08** — 마찰 제한 구간에서 $a \propto \mu$ 가 정확히 성립한다.

> [!success] ★ §8-5-1 과 나란히 놓으면 Tier 2 의 근거가 완성된다
> | | `vehicle_dynamics_st` (Tier 1) | **PhysX (Tier 2)** |
> |---|---|---|
> | 모델 | $F_y = \mu C_S(\text{하중항})\alpha$ — $\alpha$ 에 **선형** | $\lvert F_t\rvert \le \mu F_n$ — **부등식** |
> | 한계 | **없음.** $\alpha\to\infty$ 까지 힘이 증가 | **$\mu g$ 에서 정확히 막힘 (실측 $a/\mu g = 1.00$)** |
> | 슬립 | 발생하지 않음 | **0.000 → 0.766 으로 단조 증가** |
>
> ⭐ *"왜 기존 시뮬레이터를 안 쓰고 Isaac Sim 을 썼나"* 에 대한 답이
> **주장이 아니라 실측**이 되었다. $\mu$ vs $a$ 를 $\mu g$ 선과 겹쳐 그리면 논문 Figure 다.

> [!question] ⚠️ 다만 $\mu=1.0$ 의 "토크 제한" 은 아직 설명되지 않는다
> ```
> effort x4 적용 기대치  T = 4 x 2.516 = 10.06 N·m  ->  a ~ 38 m/s^2
> effort 그대로라면      T = 4 x 0.629 =  2.516 N·m  ->  a ~  9.6 m/s^2
> 실측                                                   a =   4.88 m/s^2
> ```
> 어느 쪽으로도 **절반**이다. 의심 두 가지:
> 1. `robot.actuators["drive"].effort_limit` 대입이 PhysX 까지 전파되지 않았을 수 있다.
>    USD 의 `drive:angular:physics:maxForce = 0.629` 가 살아 있으면 Python 속성 변경은 무효다.
>    스크립트가 "적용됨" 을 찍은 것은 **예외가 안 났다는 뜻일 뿐**이다.
> 2. 나머지 절반은 접촉 손실·구름 저항일 수 있다.
>
> 결론(마찰 포화)에는 영향이 없다. "토크 제한" 이라는 라벨의 근거만 약하다.

> [!tip] ⭐ 여기서 액추에이터 제어 방식 결정이 드러났다
> 현재는 **속도 서보**다: $\tau = D\,(\omega_{\text{target}} - \omega)$, $D=0.02$.
> 속도가 오르면 토크가 저절로 줄어든다. **레이싱에는 맞지 않는다.**
>
> 우리 action 은 $a_x$ 이고 friction ellipse 가 다루는 것도 **힘**이다:
> $$\tau = \frac{m\,a_x\,r}{4}\quad(\text{4륜 균등, effort 제어})$$
>
> | | 속도 서보 (현재) | **effort 제어 (권고)** |
> |---|---|---|
> | policy 출력 | 목표 휠속도 | **토크 = 힘** |
> | friction ellipse | 간접 | **직접** |
> | 실차 대응 | VESC 의 속도 루프 | **VESC 의 전류(≈토크) 제어** ✅ |
>
> 실차 ESC 는 전류 제어가 기본이고 속도 루프는 그 위에 얹힌다.
> 슬립을 학습시키려면 **힘을 직접 지령**하는 것이 맞다. → M2 에서 확정.

> [!tip] 반대로, 옳은 대응은 종방향에 있다
> $\mu=0.25$ 에서 그 코너를 도는 방법은 조향에 없다.
> $v \le \sqrt{\mu g R} = \sqrt{0.25\cdot9.81\cdot8} = 4.4\,\mathrm{m/s}$ 로 **미리** 떨어뜨리는 것뿐이다.
> ```
> 조향 조절 (횡방향, 즉각적, 한계 있음)
> 속도 조절 (종방향, 선제적, 한계 없음)   ← 지배적
> ```
> 후자는 **2초 전에** 내려야 하는 결정이다 → **$\gamma$ 와 제어 주기 측정이 중요한 이유와 직결**(§12-2).
...
| 내용 | 실차에서 실제로 얻을 수 있는 것만 | track coord, $e_{\text{lat}}$, $e_\psi$, curvature, wall distance, opponent 상대좌표 <br> **+ 동역학 파라미터 $\mu, C_{Sf}, C_{Sr}, m, h$** ← 우리가 추가 |

→ [[Asymmetric Actor-Critic (Pinto 2017)]], [[POMDP]]

> [!important] ★ TM07에 없는 것을 우리는 추가한다 — privileged obs에 동역학 파라미터
> TM07의 24-dim privileged observation은 **전부 기하 정보**다 (track coord, $e_{\text{lat}}$, $e_\psi$,
> curvature, wall distance, opponent 상대좌표). **동역학 파라미터가 하나도 없다.**
>
> 그런데 우리 연구 주제가 **마찰 변화 대응**이다. $\mu$ 가 관측에 없으면 **hidden parameter**가 되어
> 같은 $(s,a)$ 에 두 개의 정답이 생기고, critic은 **$\mu$ 분포에 대한 평균**을 배운다:
> ```
> 같은 (s,a)  :  μ=1.0 → 진짜 가치 100
>                μ=0.3 → 진짜 가치  40
> critic 학습 :  Q(s,a) ≈ E_μ[Q^μ(s,a)] ≈ 70    ← 어느 μ 에서도 최적이 아니다
> ```
> 이게 **contextual MDP / hidden-parameter MDP** 이고, *"robust 하게 만들면 느려진다"* 의 정체다.
>
> **→ critic 에 $\mu$ 를 직접 주면 $Q$ 의 모호성이 사라진다.** critic 은 배포되지 않으므로 공짜다.
> **논문에 쓸 수 있는 차별점.** → [[#13. 결정 로그]] #13, #14

> [!tip] privileged 정보의 조건 — "actor 가 원리적으로 추론 가능한 것"만
> critic 이 **actor 가 절대 구별할 수 없는 것**에 의존해 가치를 매기면, actor 는
> **실행 불가능한 행동을 향한 gradient** 를 받는다 (actor 에겐 같은 상황인데 가치가 다르다는 모순 신호).
>
> | privileged 후보 | actor 가 추론 가능? | 판정 |
> |---|---|---|
> | $\mu$, $C_{Sf}$, $C_{Sr}$ | ✅ 이력(행동↔반응)에서 | **안전** |
> | $e_{\text{lat}}$, $e_\psi$ | ✅ LiDAR + reference path | **안전** |
> | wall distance | ✅ LiDAR 에서 거의 직접 | **안전** |
> | opponent 현재 상대좌표 | ✅ LiDAR 에서 | **안전** |
> | **opponent 의 미래 의도** | ❌ 원리적으로 불가 | ⚠️ **위험** |

> [!warning] $\mu$ 는 "물리량"이 아니라 "구별 라벨"로 넘긴다
> critic 의 $\mu$ 가 실측값과 달라도 **문제가 없다.** critic 은 배포되지 않고,
> actor 는 $\mu$ 를 입력으로 받지 않는다 (이력에서 추론). $\mu$ 의 역할은
> **에피소드를 구별해 $Q$ 를 유일하게 결정하게 하는 것**뿐이다.
>
> ```python
> # ❌ 원값 (물리값 정확성에 의존하게 됨)
> critic_obs = [..., mu]                        # 1.0489
> # ✅ randomization 범위 내 상대 위치 (물리값과 무관해짐)
> mu_norm = (mu - MU_LOW) / (MU_HIGH - MU_LOW)  # 0.0 ~ 1.0
> ```
>
> **실측 $\mu$ 오차가 진짜로 아픈 곳은 critic 이 아니라
> `action_convert.py` 의 `k_fric = μg/v²` clip** 이다 — 그것만 실차에서 돌아간다.
> → 성능 한계가 아니라 **안전 봉투로 넉넉하게**(실측의 1.3배) 잡거나, 빼고 reward 로 처리.

> [!danger] replay buffer 에 저장할 것 — `next_action` 은 저장하지 않는다
> ```
> 저장 :  actor_obs, critic_obs, a, r, actor_obs', critic_obs', d
> 미저장:  next_action (ã')  ← actor 를 호출해 매번 새로 샘플링한다
> ```
> `ã'` 는 **환경이 아니라 policy 가 만든 값**이라 동역학 정보가 0 이다. 마찰 정보는 이미 $(s,a,s')$ 에 있다.
> 버퍼의 낡은 `a'` 를 쓰면 *"과거 policy 의 가치"* 를 추정하게 되어 해롭다 (그건 SARSA 이고 on-policy 다).
...
| 13 | replay buffer 에 `next_action` 을 넣어 마찰 대응을 학습해야 하나 | **아니다** | `ã'` 는 policy 가 만든 값이라 **동역학 정보가 0**. 마찰 정보는 이미 $(s,a,s')$ 에 담긴다. 낡은 `a'` 를 쓰면 과거 policy 의 가치를 추정하게 되어 해롭다 (= SARSA, on-policy) |
| 14 | ★ 그럼 마찰별 행동은 어떻게 학습하나 | **$\mu$ 를 privileged obs 에 추가** + **관측에 슬립 이력** | $\mu$ 가 관측에 없으면 hidden parameter 가 되어 critic 이 $\mu$ 분포의 **평균**을 배운다 (contextual MDP). critic 은 배포 안 되니 $\mu$ 를 직접 줘도 공짜. actor 는 $(\delta_{t-1}, \dot\psi, v_y)$ 이력에서 추론. **TM07 의 24-dim 엔 동역학 파라미터가 없다 → 우리 차별점** |
| 15 | 시뮬 $\mu$ 와 실측 $\mu$ 의 갭 | **보완할 필요 없다** | critic 은 배포되지 않고 actor 는 $\mu$ 를 받지 않는다. critic 의 $\mu$ 는 물리량이 아니라 **구별 라벨**이라 정규화해서 넘기면 절대값과 무관해진다. **진짜 갭은 ① randomization 범위 커버리지 ② $\mu$ 가 스칼라가 아님 ③ 선형 타이어 ④ actor 의 추론 구조** |
| 16 | ★ residual 의 base 를 스택 `Controller.py` 로 할 것인가 | **아니다. 순수 PP.** `Controller.py` 는 **비교군**으로 남긴다 | 코드를 직접 읽고 확인: **(2026-09-23 실측으로 강화)** solo 주행에서 출력에 영향을 주는 내부 상태만 **7개** — 처음 센 4개는 틀렸다. 결정적인 것은 `current_steer_command` 로, `calc_future_position` 이 **직전 조향각**으로 미래 위치를 예측하고 그 위치가 lookahead·횡오차·속도를 전부 정한다 = **조향이 다음 스텝 관측으로 되먹임되는 닫힌 루프**. 실측: 다른 구간 24스텝으로 오염시킨 뒤 같은 구간을 재주행하면 δ 가 4.6e-3 rad, **speed 가 2.91 m/s** 달라진다. 관측에 없는 상태가 base 출력을 바꾸면 residual 입장에서 non-Markovian — imprecise 가 아니라 unobservable 이라 더 나쁘다. 생성자 인자 40개, 배치 불가 |
| 17 | 종방향 base 를 무엇으로 채우나 | **backward/forward pass 로 오프라인 계산한 속도 프로파일.** $a_{\text{lat}} = a_{\text{lon}} = 3.5$ | **PP 는 횡방향 전용**이라 $a_x$ base 가 비어 있다. `vx_planner` 는 ggv 자리표시자라 못 쓴다(#…). 곡률만 보는 교과서 마찰원 = 파라미터 1개. **residual 에서 base 는 상한이 아니라 출발점**이므로 "vx 를 상한으로 쓰지 말라"와 충돌하지 않는다 ($\Delta a_\theta > 0$ 이면 넘어선다). 3.5 는 $\mu{=}0.357$ 에 해당 — 커리큘럼 주 구간에서 실현 가능하고 꼬리에서만 감속을 배우게 된다. **★ 런타임 규칙 $\sqrt{a/\lvert\kappa\rvert}$ 는 실측으로 폐기했다**: $\kappa_{\text{eff}}$ 를 창 안 최대로 잡으면 코너가 창에 들어오는 순간 $v_{\text{ref}}$ 가 한 격자에서 떨어져 **요구 감속이 79~156 m/s²** 가 된다(창 크기로 못 고친다). backward/forward pass 는 요구 감속·가속이 **구성상** $a_{\text{lon}}$ 으로 묶이고, 런타임 비용 0, IQP 대비 0.75~0.82배 |
| 18 | residual bound | **넓게 시작 후 데이터로 조인다** ($\Delta\kappa$ 를 $\kappa_{\max}$ 의 30% = 0.40 부터) | 이전 값 0.10 은 $\kappa_{\max}{=}1.349$ 의 **7.4%** 로 너무 좁다. 좁은 bound 는 *"policy 가 우리가 추측한 한계를 못 넘는다"* — vx 상한을 거부한 것과 **구조가 같은 문제**다. 마지막 레이어 0 초기화가 있으면 넓어도 출발점은 base 라 안전 |
| 19 | RL 코드를 스택에 넣을 것인가 | **아니다.** 신규 repo `nhg0209/rl-racing`. 스택에는 **M6 배포 노드만** | 코드를 A(학습 전용)/B(sim·real 공유)/C(실차 전용)로 나누면 B 와 C 만 실차와 관계있고, B 는 실차에 `pip install` 되는 패키지라 볼트에 둘 수 없다. **의존 방향은 실차→학습 한쪽** — 학습이 ROS 체크아웃에 의존하면 안 된다. 비교군 `Controller.py` 는 스텁 주입으로 **스택 수정 0** 으로 돌릴 수 있다 |
| 20 | 보정 레이어 ablation 사다리(L1~L4)를 돌릴 것인가 | **불필요.** 레이어별 배수를 직접 측정했다 | `tools/test_controller_adapter.py` C: 각 레이어에 1.0 을 넣어 배수를 잰다. shipped yaml 에서 `speed_adjust_lat_err` 는 **완전한 no-op**($\max\overline{\lvert\kappa\rvert}$ 가 문턱 0.8 미만), `acc_scaling` 도 scaler 가 전부 1.0 이라 정상주행 no-op. 실제 작동은 `speed_steer_scaling`(10 m/s 에서 ×0.5), `steer_scaling_for_lat_err`($2^{\text{lat\_err}}$), `speed_adjust_heading`(10° 초과) 셋 + future position + L1 단축 + heading PD. **"8겹"이 아니라 실질 6겹이고 2개는 죽어 있다** |
...
- [[기호 사전]] — 표기법
- [[트랙 생성 · Frenet 변환]] — Isaac Sim 트랙 USD + reference path + GPU Frenet projector
- [[공유 인터페이스 (sim ↔ real)]] — π_base·action·obs 규약. **학습 시작 전 전부 확정해야 함**
- [[Frenet Frame]] — $(s, d, e_\psi)$ 좌표계와 그 실패 모드
